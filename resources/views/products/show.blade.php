		...
		<div class="row w-75">
			...
			<div class="col">
				...
				@auth
						...
						<div class="row">
							...
							<div class="col-5">
								@if (Auth::user()->favorite_products()->where('product_id', $product->id)->exists())
									<a href="{{ route('favorites.destroy', $product->id) }}" class="btn ecmart-favorite-button text-favorite w-100"
					...
					<form action="{{ route('favorites.destroy', $product->id) }}" method="POST" id="favorites-destroy-form"
						class="d-none">
						@csrf
						@method('DELETE')
					</form>
					<form action="{{ route('favorites.store', $product->id) }}" method="POST" id="favorites-store-form" class="d-none">
						@csrf
					</form>
				@endauth
			</div>
					...
					<div class="row">
						<div class="offset-md-5 col-md-5">
							<form action="{{ route('reviews.store') }}" method="POST">
								...
								<h4>タイトル</h4>
								@error('content')
									<strong>タイトルを記載してください</strong>
								@enderror
								<input type="text" name="title" class="form-control m-2">
								<h4>レビュー内容</h4>
								@error('content')
									<strong>レビュー内容を記載してください</strong>
								@enderror
