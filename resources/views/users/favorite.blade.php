				...
				@else
					@foreach ($favorite_products as $favorite_product)
						<div class="row align-items-center mb-2">
							<div class="col-md-2">
								<a href="{{ route('products.show', $favorite_product->id) }}">
									@if ($favorite_product->image !== '')
										<img src="{{ asset($favorite_product->image) }}" class="img-thumbnail ecmart-product-img-cart">
									@else
							...
							<div class="col-md-6">
								<h5 class="w-100 ecmart-favorite-item-text"><a href="{{ route('products.show', $favorite_product->id) }}"
										class="link-dark">{{ $favorite_product->name }}</a></h5>
								<h6 class="w-100 ecmart-favorite-item-text mb-0">￥{{ number_format($favorite_product->price) }}</h6>
							</div>
							<div class="col-md-1">
